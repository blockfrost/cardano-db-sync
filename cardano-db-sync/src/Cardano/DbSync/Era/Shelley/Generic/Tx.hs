...
fromAlonzoTx pp (blkIndex, tx) =
  ...
  where
    ...
    getAuxScripts :: ShelleyMa.StrictMaybe (Alonzo.AuxiliaryData StandardAlonzo) -> [(ScriptHash StandardCrypto, Script (AlonzoEra StandardCrypto))]
    getAuxScripts maux = case strictMaybeToMaybe maux of
      Nothing -> []
      Just (Alonzo.AuxiliaryData _ scrs) -> map (\scr -> (Ledger.hashScript @StandardAlonzo scr, scr)) $ toList scrs
    ...
    redeemers = mkRedeemer <$> Map.toList
      (snd <$> Ledger.unRedeemers (getField @"txrdmrs" (getField @"wits" tx)))
